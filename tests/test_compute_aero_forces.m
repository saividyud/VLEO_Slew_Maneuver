...
function passed = test_cook_reference_case()
    ...
    results = vleo.aero.compute_aero_forces(objFile, location, attitude, time, options);
...
function passed = test_sentman_reference_case()
    ...
    sentmanResults = vleo.aero.compute_aero_forces(objFile, location, attitude, time, options);

    options.gsi_model = 'cook';
    cookResults = vleo.aero.compute_aero_forces(objFile, location, attitude, time, options);
...
function passed = test_position_eci_matches_geodetic_inputs()
    ...
    directResults = vleo.aero.compute_aero_forces(objFile, location, attitude, time, options);
    eciResults = vleo.aero.compute_aero_forces(objFile, struct('positionECI', positionEci), attitude, time, options);
...
function passed = test_unsupported_gsi_model_errors()
    ...
    try
        vleo.aero.compute_aero_forces(objFile, location, attitude, time, options);
        passed = false;
        fprintf('    Expected compute_aero_forces to reject unsupported gsi_model ''cll''.\n');
    catch ME
