function results = test_control_test4_exported_cases()
% TEST_CONTROL_TEST4_EXPORTED_CASES Data-driven solver regression using exported control_test4 cases.
    ...
    results = run_test(results, @test_exported_control_test4_cases);
    print_summary(results);
end

function passed = test_exported_control_test4_cases()
    caseBundlePath = fullfile(vleo.util.generated_data_dir(), 'control_test4_solver_cases.mat');
    if ~exist(caseBundlePath, 'file')
        error('test_control_test4_exported_cases:MissingCaseBundle', ...
            'Missing exported case bundle. Run scripts/export_control_test4_cases.m first.');
    end

    data = load(caseBundlePath, 'cases', 'metadata');
    if ~isfield(data, 'cases') || isempty(data.cases)
        error('test_control_test4_exported_cases:EmptyCaseBundle', ...
    ...
    fprintf('Loaded %d exported control_test4 cases from %s\n', numel(data.cases), caseBundlePath);
