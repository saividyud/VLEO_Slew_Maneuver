...
lst = lon + iauGst06(const.DJM0, Mjd_UT1, const.DJM0, Mjd_TT, NPB);
...
input.ap_a.a(1) = sw(23); % Arithmetic average of the 8 AP indices for the day

% Extract relevant Ap values from each sw data array
ap_0  = sw(15:22);        % Today's Ap values (0-3 UT)
ap_y1 = sw_1(15:22);      % Yesterday's Ap values (3-6 UT)
ap_y2 = sw_2(15:22);      % Two days ago (6-9 UT)
ap_y3 = sw_3(15:22);      % Three days ago (9-12 UT)

% Build a continuous 3-hour-bin history for the last 72 hours
ap_hist = [ap_y3, ap_y2, ap_y1, ap_0];   % Ensure ap_y3 is defined
N = numel(ap_hist);  % Total number of entries in ap_hist (should be 32)

% Identify which 3-h bin “hour” falls into today (1…8)
idxToday = floor(hour/3) + 1;    
idxToday = min(max(idxToday, 1), 8);   % Clamp to [1,8]

% Compute a global index into ap_hist
baseIdx = 24 + idxToday;  % 24 entries from the last three days + today's bin

% Set current AP value
input.ap_a.a(2) = ap_hist(baseIdx);  % Current 3-h AP

% Use modular indexing to find values for 3, 6, and 9 hours ago
for k = 1:3
    input.ap_a.a(2 + k) = ap_hist(mod(baseIdx - k - 1, N) + 1);
end

% Average of eight 3-h bins from 12 to 33 h ago
if baseIdx - 11 > 0  % Ensure we do not access negative indices
    range1 = mod(baseIdx - (4:11) - 1, N) + 1;  % Use modular indexing
    input.ap_a.a(6) = mean(ap_hist(range1));
else
    input.ap_a.a(6) = NaN;  % Assign NaN if out of bounds
end

% Average of eight 3-h bins from 36 to 57 h ago
if baseIdx - 19 > 0  % Ensure we do not access negative indices
    range2 = mod(baseIdx - (12:19) - 1, N) + 1;  % Use modular indexing
    input.ap_a.a(7) = mean(ap_hist(range2));
else
    input.ap_a.a(7) = NaN;  % Assign NaN if out of bounds
end

input.f107 = sw_1(27);   % Daily F10.7 flux for previous day (adjusted).
input.f107A = sw(28);    % Centered 81-day arithmetic average of F10.7 (adjusted).

output = gtd7d(input, flags);
dens = output.d(6); % [kg/m^3]
...
% ------------------------------- GTD7 ------------------------------ %
% ------------------------------------------------------------------- %
function [output, flags] = gtd7(input, flags)
...
tmp=input.alt;
input.alt = altt;
[soutput, flags] = gts7(input, flags);
altt = input.alt;
input.alt = tmp;
...
% ------------------------------- GTS7 ------------------------------ %
% ------------------------------------------------------------------- %
function [output, flags] = gts7(input, flags)
...
%  Diffusive density at Alt
[output.d(1), output.t(2)] = densu(z,db04,tinf,tlb, 4.0,alpha(1),output.t(2),ptm(6),s,mn1,zn1,meso_tn1,meso_tgn1);
dd = output.d(1);
