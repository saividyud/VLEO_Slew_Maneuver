function solverCase = build_too_minimax_slew_case(scenarioSeed)
    ...
    trackingHistory = vleo.analysis.compute_observation_history(tspan, xOrbitHistory, scenario.r_too_0, params);
    ...
    if isempty(visibilityInfo.idx_end_visible) || visibilityInfo.t_end_visible <= scenario.t_eruption
        error('control_test4:InvalidVisibilityWindow', ...
            'The TOO is not visible long enough after event to define a solver case.');
    end
    ...
    if idxMatch <= idxEruption || idxMatch >= visibilityInfo.idx_end_visible
        error('control_test4:SnappedMatchTimeOutOfRange', ...
            'The sampled match time is outside the valid post-event visibility interval.');
    end
    ...
    solverCase = struct();
    solverCase.caseId = sprintf('too_minimax_slew_seed_%d', scenarioSeed);
    ...
    solverCase.targetRaDeg = trackingHistory.ra_TOO_history(idxMatch);
    solverCase.targetDecDeg = trackingHistory.dec_TOO_history(idxMatch);
end
