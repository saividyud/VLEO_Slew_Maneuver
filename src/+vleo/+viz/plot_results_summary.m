function fig = plot_results_summary(results, varargin)
    parser = inputParser;
    addParameter(parser, 'FigureName', 'Results', @(x) ischar(x) || isstring(x));
    addParameter(parser, 'TitleText', 'Simulation Results', @(x) ischar(x) || isstring(x));
    ...
    fig = figure('Name', char(options.FigureName), 'NumberTitle', 'off');
    fig.WindowState = 'maximized';

    tile = tiledlayout(fig, 2, 2, 'TileSpacing', 'compact', 'Padding', 'compact');
    title(tile, char(options.TitleText), 'FontName', 'Times New Roman', 'FontSize', 16, 'FontWeight', 'bold');
