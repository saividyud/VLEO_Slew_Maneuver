%AERODYNAMICS Summary of this function goes here
...
% function [fileOut] = aerodynamics(fiName, respath, aoaS, aosS, param_eq, flag_shad, flag_sol, del, verb)
function [fileOut] = aerodynamics(X, param_eq, flag_shad, flag_sol, del)

%% Extracting angle of slip and angle of attack from X
pos = X(1:3);
vel = X(4:6);
...
o_3 = cross(o_1, o_2) / cross(o_1, o_2);

R_OI = [o_1; o_2; o_3];

R_BI = quat2dcm(X(7:10));
