...
params.aero.objFilePath = fullfile(thisFilePath, 'dynamics', '6U CubeSat.obj');
...
q0 = [q0_aerospace(2:4), q0_aerospace(1)]'; % scalar-last [x y z w]
...
simulations_dir = fullfile(fileparts(thisFilePath), 'simulations');
if ~exist(simulations_dir, 'dir')
...
fprintf('Run src/render_gif.py to generate the GIF from the latest animation data.\n');

torque_csv_path = save_torque_history_csv(thisFilePath, tspan, visibility_history, ...
...
plotActualEulerDeg = quaternion_to_euler_history_deg(X_history(:, 7:10));
ra_error = [];
dec_error = [];
tspan_verif = [];
...
if ~isnan(t_start_visible) && ~isnan(t_end_visible)
    ...
    q0_verif = [q0_aerospace(2:4), q0_aerospace(1)]';
    ...
    plotActualEulerDeg = quaternion_to_euler_history_deg(X_verif(:, 7:10));
...
plotDesiredEulerDeg = unwrap_angle_history_deg(plotDesiredEulerDeg);
plotActualEulerDeg = unwrap_angle_history_deg(plotActualEulerDeg);

% Attitude summary plot
fprintf('Generating attitude summary plot...\n');

figure('Name', 'Euler Angle Tracking', 'Position', [120 120 1500 650]);
...
function tauAero = evaluate_aero_torque(t, X, R_ECI_to_Body, params)
    ...
    try
        aeroResults = computeAeroForces(params.aero.objFilePath, location, attitude, time, params.aero.options);
...
function csvPath = save_torque_history_csv(thisFilePath, tspan, visibilityHistory, ...
        tauTotalHistory, tauControlHistory, tauAeroHistory, params, scenarioSeedText)
    simulationsDir = fullfile(fileparts(thisFilePath), 'simulations');
    if ~exist(simulationsDir, 'dir')
        mkdir(simulationsDir);
    end
...
function eulerDeg = rotation_matrix_to_euler_deg(R_B2E)
    qAerospace = dcm2quat(R_B2E');
    [yaw, pitch, roll] = quat2angle(qAerospace, 'ZYX');
    eulerDeg = rad2deg([roll, pitch, yaw]);
end

function eulerDeg = quaternion_to_euler_deg(qScalarLast)
    qScalarLast = reshape(qScalarLast, [], 1);
    qScalarLast = qScalarLast / norm(qScalarLast);
    qAerospace = [qScalarLast(4), qScalarLast(1), qScalarLast(2), qScalarLast(3)];
    [yaw, pitch, roll] = quat2angle(qAerospace, 'ZYX');
    eulerDeg = rad2deg([roll, pitch, yaw]);
end

function eulerHistoryDeg = quaternion_to_euler_history_deg(qHistoryScalarLast)
    nRows = size(qHistoryScalarLast, 1);
    eulerHistoryDeg = zeros(nRows, 3);

    for rowIdx = 1:nRows
        eulerHistoryDeg(rowIdx, :) = quaternion_to_euler_deg(qHistoryScalarLast(rowIdx, :));
    end
end
