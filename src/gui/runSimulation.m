...
%   guidata)
    ...
    % meters, so convert.
    a   = simParams.params.Orbit.semiMajorAxis * 1000; % [m]
    e   = simParams.params.Orbit.eccentricity;
    inc = deg2rad(simParams.params.Orbit.inclination);
    raan = deg2rad(simParams.params.Orbit.RAAN);
    aop  = deg2rad(simParams.params.Orbit.argPeriapse);
    ta   = deg2rad(simParams.params.Orbit.trueAnomaly);
    ...
    omega_i = deg2rad([simParams.params.Attitude.rollRate, ...
                       simParams.params.Attitude.pitchRate, ...
                       simParams.params.Attitude.yawRate]);

    %% Assemble initial state vector (13 x 1)
    X_i = [r_i, v_i, beta_i, omega_i]';

    %% Simulation time vector
    t0 = simParams.params.Simulation.initialTime;
    tf = simParams.params.Simulation.finalTime;
    dt = simParams.params.Simulation.timeStep;

    ts = t0 : dt : tf;

    %% ODE options
    opts = odeset('RelTol', simParams.params.Simulation.relTol, ...
                  'AbsTol', simParams.params.Simulation.absTol);

    %% Select dynamics model
    if strcmpi(simParams.params.Simulation.type, 'Nonlinear')
        odeFun = @Sat_template;
    else
    ...
    d = uiprogressdlg(subFigHandle, ...
        ...
        'Message', sprintf('Integrating %s equations of motion...', ...
                           simParams.params.Simulation.type), ...
        'Indeterminate', 'on');
